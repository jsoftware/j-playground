...
function dlog_add(t) {
 ...
 if (Dlog.length == DlogMax) Dlog.splice(0, 1);
 Dlog.push(t);
 DlogPos = Dlog.length;
 dlog_write();
}
...
function dlog_select() {
 var n = Dlog.length;
 if (n == 0) return msgbox("No input log");

 var n = Dlog.length;
 var h = `<table id="dlogtop">
<tr><td>Input Log</td><td style='width:100%'></td>
<td class='vtop'><div id='popup-close'>&#215;
</div></td></tr></table>`;
 h += "<select id='dlogsel' size=" + Math.min(20, n) + ">";
 for (var i = 0; i < n; i++)
  h +=
   "<option " + (i === n - 1 ? "selected" : "") + ">" + Dlog[i] + "</option>";
 h += "</select>";
 popup(h, 400);
